...
use crate::{ AppState, DISTRIBUTION_DENOMINATOR, FOUNDER_AIRDROP_PERCENT };
...
pub fn airdrop_founder_handler(ctx: Context<AirdropFounder>) -> Result<()> {
    let amount: u64 =
        (ctx.accounts.app_state.founder_amount / (DISTRIBUTION_DENOMINATOR as u64)) *
        (FOUNDER_AIRDROP_PERCENT as u64);
    let seeds: &[&[u8]; 3] = &[
        b"authority",
